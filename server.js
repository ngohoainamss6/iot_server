...
app.post("/api/save", async (req, res) => {
  const { soil, temp, hum, flow, mode, min, max, next, pump_power, schedule } =
    req.body;
  try {
    await pool.query(
...
app.post("/api/control", async (req, res) => {
  ...
  try {
    ...
    if (last.schedules) {
      try {
        schedules = JSON.parse(last.schedules);
      } catch (e) {
        schedules = [];
      }
    }
    ...
    const newPump =
      typeof pump === "boolean" ? pump : last?.pump ?? false;
    ...
    // 2️⃣ Cập nhật ngay trạng thái UI (system_status)
...
app.get("/api/command", async (req, res) => {
  try {
    ...
    if (cmd.schedules)
      cmdStr += "SCHEDULES:" + JSON.stringify(cmd.schedules) + ";";
